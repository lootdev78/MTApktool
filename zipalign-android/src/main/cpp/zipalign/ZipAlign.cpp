...
static int getAlignment(int sharedLibraryPageAlignment, int defaultAlignment,
    ZipEntry* pEntry) {

    if (sharedLibraryPageAlignment <= 0) {
        return defaultAlignment;
    }

    const char* ext = strrchr(pEntry->getFileName(), '.');
    if (ext && strcmp(ext, ".so") == 0) {
        return sharedLibraryPageAlignment;
    }
 ...
 */
static int copyAndAlign(ZipFile *pZin, ZipFile *pZout, int alignment, int sharedLibraryPageAlignment) {
    ...
    for (int i = 0; i < numEntries; i++) {
        ...
        if (pEntry->isCompressed() || isDirectory(pEntry)) {
            ...
            const int alignTo = getAlignment(sharedLibraryPageAlignment, alignment, pEntry);
 ...
 */
extern "C"
int process(const char *inFileName, const char *outFileName, int alignment, int sharedLibraryPageAlignment, bool force) {
    ZipFile zin, zout;
    ...
    int result = copyAndAlign(&zin, &zout, alignment, sharedLibraryPageAlignment);
    if (result != 0) {
 ...
 */
extern "C"
int verify(const char *fileName, int alignment, int sharedLibraryPageAlignment, bool verbose) {
    ZipFile zipFile;
    bool foundBad = false;
