...
object SplitArchiveSupport {
    ...
    fun isSplitArchive(file: File): Boolean = file.isFile && file.extension.lowercase(Locale.ROOT) in supportedExtensions
    ...
    fun isSplitDecodeCommand(command: String): Boolean {
        ...
        var i = 0
        if (args[0].equals("apktool", true)) i++
        return args.getOrNull(i)?.let { it.equals("apks-decode", true) || it.equals("split-decode", true) } == true
    }

    @Throws(Exception::class)
    fun executeDecode(command: String, toolchain: Toolchain, listener: ApktoolCommandRunner.Listener): ApktoolCommandRunner.Result {
        ...
        if (parsed.allSplits) {
            if (!outputRoot.isDirectory && !outputRoot.mkdirs()) throw IOException("Cannot create output directory: $outputRoot")
        } else {
            ...
            ZipFile(archive).use { zip ->
                ...
                selected.forEachIndexed { index, entry ->
                    ...
                    val decodeArgs = buildList {
                        add("apktool"); add("decode"); addAll(parsed.decodeFlags)
                        add("-o"); add(target.absolutePath); add(extracted.absolutePath)
                    }
    ...
    private fun parse(command: String): Parsed {
        ...
        if (args.firstOrNull()?.equals("apktool", true) == true) args.removeAt(0)
        ...
        while (i < head.size) {
            when (val token = head[i]) {
                ...
                "--entry" -> { entry = head.getOrNull(i + 1) ?: throw IllegalArgumentException("--entry requires a ZIP entry path"); i++ }
                else -> positional += token
            }
            i++
        }
        if (positional.size != 2) throw IllegalArgumentException("apks-decode [--all-splits | --entry name.apk] <archive> <output-dir> -- [decode options]")
    ...
    private fun choosePreferredZip(entries: List<ZipEntry>): ZipEntry = entries.maxBy { score(ApkEntry(it.name, it.size)) }
    ...
    private fun score(entry: ApkEntry): Long {
        ...
        when {
            ...
            name == "base.apk" || (!name.startsWith("config") && !name.startsWith("split")) -> score += 700_000
        }
