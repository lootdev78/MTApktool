...
private fun GeneralSettingsDialog(onBack: () -> Unit) {
    ...
    val theme by manager.themeModeFlow.collectAsState(initial = ThemeMode.SYSTEM)
    ...
    AlertDialog(
        ...
        text = {
            Column {
                Text("Darstellung", fontWeight = FontWeight.SemiBold)
                ThemeMode.entries.forEach { mode ->
                    ...
                    TextButton(
                        onClick = { scope.launch { manager.setThemeMode(mode) } },
                        modifier = Modifier.fillMaxWidth(),
                    ) { Text(if (theme == mode) "✓  $label" else label) }
                }
                Text(
                    "Dateizugriff, Job-Benachrichtigungen und die Apktool-Runtime werden über Android-Berechtigungen und Erstellen & Dekodieren gesteuert.",
