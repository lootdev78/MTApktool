...
public final class ResXmlUtils {
    ...
    /** Returns true when the application already declares android:networkSecurityConfig. */
    public static boolean hasNetworkSecurityConfig(File file) {
        try {
            Document doc = XmlUtils.loadDocument(file);
            Node application = doc.getElementsByTagName("application").item(0);
            if (application == null) return false;
            NamedNodeMap attrs = application.getAttributes();
            return attrs != null && attrs.getNamedItem("android:networkSecurityConfig") != null;
