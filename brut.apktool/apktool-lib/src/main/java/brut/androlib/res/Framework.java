...
public class Framework {
    ...
    static {
        String androidDir = System.getProperty("apktool.android.framework.dir", "");
        if (!androidDir.isEmpty()) {
            DEFAULT_DIRECTORY = new File(androidDir);
        } else {
    ...
    public File getApkFile(int id, String tag) throws AndrolibException {
        String selectedFramework = mConfig.getFrameworkFile();
        if (id == 1 && selectedFramework != null && !selectedFramework.isEmpty()) {
            File selected = new File(selectedFramework);
            if (selected.isFile() && selected.canRead()) {
                return selected;
            }
            throw new AndrolibException("Selected framework APK is not readable: " + selectedFramework);
        }
