...
public class ApkBuilder {
    ...
    private void buildResourcesFully(File outDir, File manifest, File resDir) throws AndrolibException {
        ...
        if (mConfig.isNetSecConf()) {
            boolean keepExisting = mConfig.isNetSecConfKeepExisting()
                    && ResXmlUtils.hasNetworkSecurityConfig(manifest);
            if (keepExisting) {
                Log.i(TAG, "Keeping existing network security config unchanged...");
            } else {
