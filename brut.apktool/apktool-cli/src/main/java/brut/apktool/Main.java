...
public class Main {
    ...
    private static final Option decodeUseRegistersOption = Option.builder()
        .longOpt("use-registers")
        .desc("Emit .registers instead of .locals in decoded smali.")
        .get();
    ...
    private static Options generalOptions = new Options();
    private static Options decodeOptions = new Options();
    private static Options buildOptions = new Options();
    private static Options installFrameworkOptions = new Options();
    private static Options cleanFrameworksOptions = new Options();
    private static Options listFrameworksOptions = new Options();
    private static Options publicizeResourcesOptions = new Options();

    private static final Props props = new Props();
    private static Config config = new Config(props.getVersion());
    ...
    /** Android-safe entry point. It never terminates the hosting app process. */
    public static void main(String[] args) throws AndrolibException {
        run(args);
    }

    /** Runs the original Apktool CLI parser and returns the would-be process exit code. */
    public static synchronized int run(String[] args) throws AndrolibException {
    ...
    private static final class CliExit extends RuntimeException {
        final int code;
        CliExit(int code) {
            super("Apktool CLI exit " + code, null, false, false);
            this.code = code;
        }
    }

    private static void executeMain(String[] args) throws AndrolibException {
    ...
    private static void cmdDecode(String[] args) throws AndrolibException {
        ...
        if (cli.hasOption(decodeUseRegistersOption)) {
            if (cli.hasOption(decodeNoSrcOption)) {
                printOptionConflict(decodeUseRegistersOption, decodeNoSrcOption);
            } else {
                config.setBaksmaliUseRegisters(true);
            }
        }
